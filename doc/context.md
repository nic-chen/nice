...
nice 集成一个简单的模板渲染，使用Go标准库的 [template语法](https://godoc.org/html/template)。

nice 的模板渲染使用 `Context`存储 中的数据作为模板变量。
...
import nice "github.com/nic-chen/nice"
...
nice 中运行通过 `DI` 替换模板引擎，只要实现 `nice.Renderer` 接口即可。
