...
func TestRun1(t *testing.T) {
	Convey("run nice app", t, func() {
		Convey("run nice app normal", func() {
			b3 := New()
			go b3.Run(":8011")
		...
		Convey("run nice app error", func() {
			b3 := New()
			defer func() {
				e := recover()
				So(e, ShouldNotBeNil)
...
func TestServeHTTP1(t *testing.T) {
	Convey("ServeHTTP", t, func() {
		...
		Convey("error serve", func() {
			n.SetError(func(err error, c *Context) {
				c.Resp.WriteHeader(500)
				c.Resp.Write([]byte(err.Error()))
			})
			n.Get("/error", func(c *Context) {
